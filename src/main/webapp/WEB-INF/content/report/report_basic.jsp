	    	...
	    	<c:if test="${not empty compareSelectRange}">
		    	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table panel-body table-bordered " >
		         <tbody>
		           ...
		           <th>创角用户 <i class="helpToolTip" data-content="统计所选时期内，每日玩家激活游戏后，进行了注册并创建了游戏角色的账户数量"></i></th>
							<th>注册用户 <i class="helpToolTip" data-content="统计所选时期内，每日玩家激活游戏后，进行了注册有ID信息或者账户信息的玩家账户数量"></i></th>
							<th>注册设备 <i class="helpToolTip" data-content="当日玩家激活游戏后，进行了注册有ID信息或者账户信息的玩家设备数量"></i></th>
							...
							<c:if test="${empty channelIds && empty compareChannelIds && empty zoneIds && empty compareZoneIds }">
								<th>总游戏启动次数 <i class="helpToolTip" data-content="以设备为唯一，记录用户启动游戏总次数；游戏启动至加载完我们SDK后开始记录启动次数"></i></th>
							</c:if>
							<th>充值额 <i class="helpToolTip" data-content="统计所选时期内，每日玩家（账号为唯一）成功充值的金额总值。包括新老用户的充值"></i></th>
		           ...
		           <tr>
		           		...
		           		<fmt:formatNumber  value="${result['data'][0].avgOnlineTime}" pattern="0"  var="total"/>
				          				...
				          				<fmt:formatNumber  value="${(total-totalHour*3600)/60-0.5+ 0.0001}" pattern="0"  var="totalMinute"/>
				          		
				          				
				          				<fmt:formatNumber  value="${total-totalHour*3600-totalMinute*60}" pattern="0"  var="totalSecond"/>
		           		...
		           		<c:if test="${result['data'][0].activeUsers > 0 }">
		           			<td><fmt:formatNumber  value="${result['data'][0].payAmount/100/result['data'][0].activeUsers }" pattern="0.00" /></td>
		           		</c:if>
		           		<c:if test="${result['data'][0].activeUsers == 0 }">
		           			<td>0.00</td>
		           		</c:if>
		            ...
		            <tr>
		           		...
		           		<td><fmt:formatNumber  value="${result['compareData'][0].payAmount/100 }" pattern="0.00" /></td>
		           		...
		           		<fmt:formatNumber  value="${result['compareData'][0].avgOnlineTime}" pattern="0"  var="total"/>		
				          				...
				          				<td>
				          					<fmt:formatNumber  value="${result['compareData'][0].avgOnlineTime/3600 }" pattern="0.00" />
				          				</td>
		           		...
		           		<c:if test="${result['compareData'][0].activeUsers > 0 }">
		           			<td><fmt:formatNumber  value="${result['compareData'][0].payAmount/100/result['compareData'][0].activeUsers }" pattern="0.00" /></td>
		           		</c:if>
		           		<c:if test="${result['compareData'][0].activeUsers == 0 }">
		           			<td>0.00</td>
		           		</c:if>
	      		...
	      		<table class="table table-hover table-striped table-bordered table-condensed table-big">
          			<thead>
            			<tr>
							...
							<th>活跃用户 <i class="helpToolTip" data-content="统计所选时期内，每日成功登录游戏的老玩家（账号唯一）数量"></i></th>
							<c:if test="${empty channelIds && empty compareChannelIds && empty zoneIds && empty compareZoneIds }">
								<th>游戏启动次数 <i class="helpToolTip" data-content="以设备为唯一，记录用户启动游戏总次数；游戏启动至加载完我们SDK后开始记录启动次数"></i></th>
							</c:if>
							<th>充值额 <i class="helpToolTip" data-content="统计所选时期内，每日玩家（账号为唯一）成功充值的金额总值。包括新老用户的充值"></i></th>
							<th>充值人数 <i class="helpToolTip" data-content="统计所选时期内，每日成功充值的用户（账号唯一）数量"></i></th>
							<th>新用户充值额 <i class="helpToolTip" data-content="统计所选时期内，当天新用户（账号唯一）充值总额"></i></th>
								...
								<s:iterator value="result['data']" var="item" >
									...
									<s:set var="allnewUserPays" value="0"> </s:set>
				          			<s:iterator value="#item.value" var="itemVal" status="st">
				          				...
				          				<s:set var="allnewActiveDevices" value="#allnewActiveDevices + #itemVal.newActiveDevices"></s:set>
				          				<s:set var="allactiveUsers" value="#allactiveUsers + #itemVal.activeUsers"></s:set>
				          				<s:set var="allstartTimes" value="#allstartTimes + #itemVal.startTimes"></s:set>
				          				...
				          				<s:set var="allavgOnlineTime" value="#allavgOnlineTime + #itemVal.avgOnlineTime"></s:set>
					          			<tr>
											...
											<s:else>
												<s:set value="#itemVal.avgOnlineTime" var="totalSecond"></s:set>
		           		                        <s:set value="#totalSecond/3600" var="hour"></s:set>
		           		                        <s:set value="(#totalSecond-#hour*3600)/60" var="minute"></s:set>
		           		                        <s:set value="#totalSecond-#hour*3600-#minute*60" var="second"></s:set>
		           		                        
		           		                        <td>
		           		                        	<fmt:formatNumber  value="${itemVal.avgOnlineTime/3600 }" pattern="0.00" />
		           		                        </td>
											</s:else>
											...
											<c:if test="${activeUsers > 0 }">
												<td><fmt:formatNumber  value="${payAmount/100/activeUsers }" pattern="0.00" /></td>
											</c:if>
											<c:if test="${activeUsers == 0 }">
												<td>0.00</td>
											</c:if>
				          			...
				          			<tr>
				          				...
				          				<fmt:formatNumber  value="${allavgOnlineTime/itemSize }" pattern="0"  var="total"/>
				          				...
				          				<td>
				          					<fmt:formatNumber  value="${allavgOnlineTime/3600 }" pattern="0.00" />
				          				</td>
				          				...
				          				<c:if test="${allactiveUsers > 0 }">
				          					<td><fmt:formatNumber  value="${allpayAmount/100/allactiveUsers }" pattern="0.00" /></td>
				          				</c:if>
				          				<c:if test="${allactiveUsers == 0 }">
				          					<td>0.00</td>
				          				</c:if>
						...
						<s:if test="result['data'].size > 0">
							<s:iterator value="result['data']" var="item" status="st">
								...
								<tr>
									...
									<td><fmt:formatNumber  value="${result['compareData'][st.index].payAmount /100 }" pattern="0.00" /></td>
									...
									<fmt:formatNumber  value="${result['compareData'][st.index].avgOnlineTime }" pattern="0"  var="total"/>
				          			...
				          			<fmt:formatNumber  value="${total-totalHour*3600-totalMinute*60}" pattern="0"  var="totalSecond"/>           		
 		           		            ...
 		           		            <td>
				          				 <fmt:formatNumber  value="${result['compareData'][st.index].avgOnlineTime/3600 }" pattern="0.00" />
				          			</td>
									...
									<c:if test="${result['compareData'][st.index].activeUsers > 0 }">
										<td><fmt:formatNumber  value="${result['compareData'][st.index].payAmount/100/result['compareData'][st.index].activeUsers }" pattern="0.00" /></td>
									</c:if>
									<c:if test="${result['compareData'][st.index].activeUsers == 0 }">
										<td>0.00</td>
									</c:if>
