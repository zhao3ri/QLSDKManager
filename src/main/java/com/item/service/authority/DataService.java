...
public class DataService {
    ...
    public String getDataListHtml(Long moduleId, Long identityId) {
        int count = 0;
        Long level = 4l;
        StringBuffer sb = new StringBuffer();
        List<Data> dataList = this.getDataListByModuleId(moduleId);
        List<IdentityData> identityDataList = null;
        if (identityId != null)
            identityDataList = rds.getIdentityDataListById(identityId);
        sb.append("<table width=\"60%\">")
                .append("<tr>");
        for (Data data : dataList) {
            if (identityDataList != null)
                level = this.getRoleDataSelectedLevel(data.getId(), identityDataList);
            sb.append("<td >")
                    .append(data.getDatasetName())
                    .append("</td>")
