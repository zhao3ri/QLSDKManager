...
public class FunctionService{
	...
	public void list2map(List<Function> functionList){
		if(functionList!=null && !functionList.isEmpty()){
			for (Function function:functionList) {
				AuthCacheManage.functionCacheMap.put(function.getId(), function);
			}
		}
	}
	...
	public Function getFunctionByID(Long id){
		if(AuthCacheManage.functionCacheMap.isEmpty()){
			return null;
		}
		return AuthCacheManage.functionCacheMap.get(id);
	}
	
	public Function getFunctionByName(String functionName){
		if(AuthCacheManage.functionCacheList!=null && !AuthCacheManage.functionCacheList.isEmpty()){
			for(Function fun:AuthCacheManage.functionCacheList){
				if(fun.getFunctionName().equals(functionName)){
					return fun;
				}
			}
		}
	...
	public void delete(String checkedIds) throws Exception{
		for(String checkedId : StringUtils.split(checkedIds,",")){
			...
			roleAuthService.deleteByAuthID(auth.getId());
